...
class Streamer(object):
    ...
    def _get_username(self):
        ...
        name=StringVar()
        entry_box= Entry(root, textvariable=name, width=25, bg="steelblue").place(x=200, y=203)
        print("setting up")
        def do_it():
            self.train_name = str(name.get())
            # root.quit()
        work= Button(root, text="ENTER", width=30, height=5, bg="steelblue", command=do_it).place(x=250,y=300)
        root.mainloop()